...
// magic numbers
#define stdfile="StudentRecords..txt"
#define profile="ProgrammeRecords.txt"
...
bool loadofile(ifstream &file, const char *filename, int count){
    if (fexists(filename)){
        ...
        string msg="\n"+(string)filename+"not accessable!";
        if (count>1){
            msg+=" retrying "+to_string(count)+" time";
            msg=(count>1)?msg:msg+"s";
            return loadifile(file,filename,count);
        } else {
            return false;
        }
...
void loadfromfile(){
    cout<<"loadfilefile";
    ifstream stdfile;
    if (loadifile(stdfile,"StudentRecords..txt")) {
        cout<<"\nok";
    } else {
        cout<<"\nnot ok";
    }
/*
ProgrammeRecords.txt
StudentRecords.txt
*/
}
